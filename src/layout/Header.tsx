...
export const Header: React.FC = () => {
  ...
  const navLinks = [
    ...
    { name: 'Our Pricelist', action: 'openPrices' },
    { name: 'Why Gran Maestro', path: '/why-us' },
    { name: 'From The Founder', path: '/founder' },
      ...
      <header className={`fixed left-0 right-0 z-50 transition-all duration-500 ease-out flex justify-center px-4 sm:px-6 lg:px-8 ${isScrolled ? 'top-4' : 'top-6'}`}>
        <div className={`flex justify-between items-center w-full max-w-6xl transition-all duration-500 ${isScrolled ? 'glass dark:glass-dark px-4 py-2 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.1)]' : 'bg-transparent py-2 px-0 rounded-none'}`}>
          ...
          <div className="hidden md:flex items-center gap-3">
            <button onClick={toggleDarkMode} className="p-2.5 rounded-full hover:bg-gray-200 dark:hover:bg-brandBlue-800 transition-colors text-brandBlue-900 dark:text-gray-200 border border-transparent hover:border-gray-300 dark:hover:border-brandBlue-600">
              {isDarkMode ? (
          ...
          <div className="md:hidden flex items-center gap-3 z-50">
            <button onClick={toggleDarkMode} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-brandBlue-800 transition-colors text-brandBlue-900 dark:text-gray-200 bg-white/50 dark:bg-black/20 backdrop-blur-sm">
              {isDarkMode ? (
            ...
            <button 
              ...
              className="text-brandBlue-900 dark:text-gray-200 focus:outline-none p-2 bg-white/50 dark:bg-black/20 rounded-full backdrop-blur-sm shadow-sm"
            >
              {isMobileMenuOpen ? (
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            ...
            className="md:hidden fixed inset-0 z-40 bg-white/95 dark:bg-[#041333]/95 backdrop-blur-xl pt-24 pb-8 px-6 flex flex-col items-center shadow-2xl"
          >
